...
Triggers define how workflows are initiated. There are three types:
...
HTTP endpoint that starts the workflow when called. Configuration:
- **method**: HTTP method to accept (GET, POST, etc.)
- **schema**: Optional JSON Schema for validating the webhook payload

The webhook path is generated by the engine (typically a UUID), not specified in the trigger configuration. This prevents path collisions and provides security through obscurity.

### Component Trigger

A WebAssembly component that can register webhooks or poll external services. Component triggers implement a WIT interface with:
- **start()**: Called when the trigger is activated
- **handle()**: Called to process incoming events
- **stop()**: Called when the trigger is deactivated

Component triggers receive pre-resolved inputs conforming to the trigger's schema (defined in the component manifest).
...
When a task executes, it receives a `TaskContext` with:
- **execution_id**: Unique identifier for this workflow execution
- **node_id**: Identifier for the node in the workflow graph
- **task_id**: Unique identifier for this specific task execution
- **inputs**: Pre-resolved input values (path expressions like `$.node.output.field` are resolved by the engine before reaching the task)

### Task Output

Tasks produce a `TaskOutput` containing:
- **output**: JSON output data
- **artifacts**: References to any artifacts produced
