...
impl fuschia::kv::kv::Host for TaskHostState {
...
impl fuschia::config::config::Host for TaskHostState {
  fn get(&mut self, key: String) -> Option<String> {
    self.inner.config_get(&key).cloned()
  }
}

impl fuschia::log::log::Host for TaskHostState {
...
pub use exports::fuschia::task::task::{Context, Output};
