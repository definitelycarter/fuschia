...
//! and executes workflows using the [`Runtime`].

use std::sync::Arc;

use fuschia_runtime::{InvokeResult, Runtime, RuntimeError};
use tokio::sync::mpsc;
...
impl WorkflowRunner {
  ...
  ///
  /// # Arguments
  /// * `runtime` - The runtime for executing workflows
  pub fn new(runtime: Arc<Runtime>) -> Self {
    Self::with_buffer_size(runtime, 100)
  }

  /// Create a new workflow runner with a custom buffer size.
  pub fn with_buffer_size(runtime: Arc<Runtime>, buffer_size: usize) -> Self {
  ...
  pub async fn start(mut self, cancel: CancellationToken) -> Result<(), RuntimeError> {
    let workflow = self.runtime.workflow();
    info!(
        ...
        payload = self.receiver.recv() => {
          match payload {
            Some(payload) => {
              ...
              match self.runtime.invoke(payload, exec_cancel).await {
                Ok(result) => {
                  info!(
                    workflow_id = %self.runtime.workflow().workflow_id,
  ...
  pub async fn execute_once(
    ...
    self.runtime.invoke(payload, cancel).await
  }
...
mod tests {
  use super::*;
  use fuschia_runtime::RuntimeConfig;
  ...
  fn create_test_runtime() -> Arc<Runtime> {
    ...
    Arc::new(Runtime::new(workflow, config).unwrap())
  }
