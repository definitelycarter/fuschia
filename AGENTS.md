...
- `crates/` - Workspace member crates
  ...
  - `fuscia-trigger` - Trigger types for workflow initiation. Defines `Trigger` enum (Manual, Webhook, Component) and `TriggerEvent` for starting workflow executions. Webhook paths are generated by the engine, not specified in config.
