...
- `crates/` - Workspace member crates
  ...
  - `fuschia-engine` - Workflow execution engine. Contains `WorkflowEngine` for graph traversal and task execution, `WorkflowRunner` for channel-based triggering, and minijinja-based input resolution. Executes nodes in parallel when dependencies are satisfied. Supports cancellation via `CancellationToken`.
